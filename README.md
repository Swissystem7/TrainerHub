...
- **ליבה משותפת** (`js/core.js` + `js/catalog.json`): סכימת `phases`, `esc`, אחסון, `wa.me`, מנוע התוכנית, וקטלוג סרטונים אמיתיים (`file` = שם mp4). מנוע התוכנית בוחר ממאגר תרגילים נפרד — בלי שמות קובץ מזויפים. בדיקות: `node --test` (בלי תלויות) על המנוע, הקטלוג, קוד הסיום ל־`wa.me`, וסכימת `phases`.
...
- **מצב אימון:** טיימר אחד לכל שלב, דילוג על סט בודד, חזרה שמורידה מונים. אם יש `file` ב־`js/catalog.json` מוצג `<video>`; הקליפים עצמם מחוץ לריפו.
- **לינק מתאמן:** התוכנית ב־hash של הכתובת, בלי התחברות. בסיום — סיכום ל־`wa.me` וקוד `TH1` לייבוא.
- **יומן ביצוע:** ייבוא הקוד מהוואטסאפ, רשת 28 יום, בלי לקוחות מזויפים. **הצע שבוע הבא** משנה סטים לפי קושי/דילוגים.
