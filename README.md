...
**[דמו חי על GitHub Pages](https://swissystem7.github.io/TrainerHub/)** · [ספריית הסרטונים](https://swissystem7.github.io/TrainerHub/library.html) · [האפליקציה המלאה](https://swissystem7.github.io/TrainerHub/frontend/) · [מצב אימון](https://swissystem7.github.io/TrainerHub/frontend/workout-mode.html)
...
- **ליבה משותפת** (`js/core.js` + `js/catalog.json`): סכימת `phases`, `esc`, אחסון, `wa.me`, מנוע התוכנית, וקטלוג סרטונים אמיתיים (`file` = שם mp4). כשהקטלוג טעון המנוע **מעדיף קליפים אמיתיים** על מאגר גנרי. חיפוש, תגיות (ילדים / זוגות / ספורט), חלופות, ולינק לקליפ בודד (`#CLIP.`) — בלי שם קובץ בכתובת. בדיקות: `node --test` (בלי תלויות) על המנוע, הקטלוג, הספרייה, קוד הסיום ל־`wa.me`, וסכימת `phases`.
- **ספריית סרטונים** (`library.html`): 78 קליפים בעברית — חיפוש, סינון לפי שריר/ציוד/קהל, שליחה בוואטסאפ, והרכבת אימון מהספרייה. הנגן מופיע רק אם הקובץ נמצא ב־`videos/`. שמות קבצים לא מוצגים בממשק.
...
- **מצב אימון:** טיימר אחד לכל שלב, דילוג על סט בודד, חזרה שמורידה מונים. אם יש `file` ב־`js/catalog.json` מוצג `<video>`; הקליפים עצמם מחוץ לריפו. **החלף** מציע חלופה מאותה קבוצת שרירים. לינק `#CLIP.` פותח תרגיל בודד מהספרייה.
...
אין סליקה, אין וואטסאפ־שרת, ואין ספריית 95 סרטונים בריפו. `backend/` אופציונלי ולא רץ ב־Pages. מסמכי האקתון/תמחור ישנים ב־[docs/archive/](docs/archive/INDEX.md).
